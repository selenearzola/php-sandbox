...
function get_registered_users($path = MAILING_LIST)
{
	...
	$users = file($path);
	// If there's a single username, we create an PHP callback function

	if (count($users)) {

		// We return an array_map wich contains all the users mail list information.
		// Then we explode it, by adding a ":" to separate username from email address. Also we're using htmlspecialchars to avoid the html injection.
...
{
	// Returns true: if the email it passes the validation or false: if the email doesn't pass the validation.
